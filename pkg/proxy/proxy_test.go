...
func TestGracefulCloseConn(t *testing.T) {
	...
	createClientConn := func() *client.ClientConnection {
		...
		require.NoError(t, err)
		clientConn := client.NewClientConnection(lg, conn, nil, nil, hsHandler, 0, "", 0, &backend.BCConfig{})
...
func TestWatchCfg(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		...
		return server.mu.requireBackendTLS == cfg.Security.RequireBackendTLS &&
			...
			metrics.ShouldCollectQueryInteractionForUser("app_0") &&
			metrics.ShouldCollectQueryInteractionForUser("readonly") &&
			!metrics.ShouldCollectQueryInteractionForUser("root") &&
