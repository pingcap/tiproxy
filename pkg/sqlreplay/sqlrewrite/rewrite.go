...
// MaybeRewrite returns rewritten SQL when the replay digest matches and the SQL contains the tiflash hint.
func (r *Rewriter) MaybeRewrite(sql string) (string, bool) {
	if r == nil || len(r.digestAllowlist) == 0 || !tiflashReadHintRE.MatchString(sql) {
		return sql, false
	}
