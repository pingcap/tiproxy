...
// I do not call the package directly because the errors are not returned, but printed in glog.
...
func arpSendGratuitous(link string, ip net.IP) error {
	iface, err := net.InterfaceByName(link)
	if err != nil {
		return fmt.Errorf("failed to get interface %q: %v", link, err)
	}
